...
- Identity state is `WorktreeIdentity` with `schema_version`, `did`, `stable_id`, address, custody, canonical `identity_scope`, registry URL/status, and created-at; its file decoder alone accepts deprecated pre-v2 `lifetime` input (`awconfig/identity.go`).
