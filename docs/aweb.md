...
# Start the service
uv run aweb serve --host 0.0.0.0 --port 8001 --reload
```
...
```bash
export AWEB_DATABASE_URL=postgresql://beadhub:dev-password@localhost:5432/beadhub
uv run aweb seed --project-slug conformance --aweb-url http://localhost:8001
```

To seed a second project identity (enables cross-project scoping conformance tests), add:

```bash
uv run aweb seed --project-slug conformance --aweb-url http://localhost:8001 --other-project-slug conformance-other
```
