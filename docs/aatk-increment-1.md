...
| Rows and fixture receipts are typed and source-bound | canonical schema and lifecycle fixture tests | blank/sentinel, unchecked command, wrong SHA/digest/app/context, mixed run, expiry, terminal, unrelated failure, duplicate/unknown/missing receipt tests |
...
| Preplan and release-close cannot be authorized by this increment | schema validation passes | independent preplan and release tests both fail with `unenforced-obligation` and the full deferred-ID set |
