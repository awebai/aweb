...
-- Opaque E2E message storage. Existing rows are legacy plaintext v1.
...
ALTER TABLE {{tables.messages}}
    ADD CONSTRAINT messages_content_mode_valid
...
CREATE INDEX IF NOT EXISTS idx_messages_content_mode
    ON {{tables.messages}} (content_mode, created_at);

CREATE INDEX IF NOT EXISTS idx_messages_signed_envelope_hash
    ON {{tables.messages}} (signed_envelope_hash)
