"""Aweb coordination team roles endpoints."""
...
DEFAULT_TEAM_ROLES_BUNDLE: Dict[str, Any] = get_default_bundle()
...
class TeamRolesBundle(BaseModel):
    """Versioned team roles bundle containing roles and adapters."""

    roles: Dict[str, Dict[str, Any]] = Field(default_factory=dict)
    adapters: Dict[str, Any] = Field(default_factory=dict)


class TeamRolesVersion(BaseModel):
...
async def create_team_roles_version(
    ...
    *,
    ...
    """Create a versioned team roles record for a team."""
    result = await db.fetch_one(
        """
    ...
    if not result:
        ...
        raise HTTPException(
            status_code=409,
            detail=(
                f"Team roles conflict: base_roles_id {base_roles_id} "
                f"does not match active team roles {active_id}. "
                f"Another agent may have updated the team roles. "
                f"Re-read the active team roles and retry."
            ),
        )
...
class ActiveTeamRolesResponse(BaseModel):
...
class CreateTeamRolesRequest(BaseModel):
    """Request body for POST /v1/roles."""

    bundle: TeamRolesBundle = Field(
        ...,
        description="Team roles bundle containing roles and adapters.",
    )
    base_team_roles_id: Optional[str] = Field(
        None,
        description="Optional bundle ID that this version is based on.",
    )


class CreateTeamRolesResponse(BaseModel):
...
class ActivateTeamRolesResponse(BaseModel):
    """Response for POST /v1/roles/{id}/activate."""

    activated: bool
    active_team_roles_id: str


class ResetTeamRolesResponse(BaseModel):
...
class DeactivateTeamRolesResponse(BaseModel):
...
class TeamRolesHistoryItem(BaseModel):
    """A team roles version in the history list."""
...
class TeamRolesHistoryResponse(BaseModel):
    """Response for GET /v1/roles/history."""

    team_roles_versions: List[TeamRolesHistoryItem]


@roles_router.get("/active")
async def get_active_team_roles_endpoint(
    request: Request,
    response: Response,
    ...
    etag = _generate_etag(
        ...
        team_roles_version.updated_at or team_roles_version.created_at,
    )
    ...
    available_roles = list(team_roles_version.bundle.roles.keys())
    ...
    if selected_role_name:
        if selected_role_name not in team_roles_version.bundle.roles:
            raise HTTPException(
                status_code=400,
                detail=f"Role '{selected_role_name}' not found. Available roles: {available_roles}",
            )
        role_info = team_roles_version.bundle.roles[selected_role_name]
    ...
    if only_selected:
        ...
        roles = {
            selected_role_name: RoleDefinition(
                **team_roles_version.bundle.roles[selected_role_name]
            )
        }
    else:
        roles = {
            ...
            for name, info in team_roles_version.bundle.roles.items()
        }
...
async def list_team_roles_history(
    request: Request,
    ...
    return TeamRolesHistoryResponse(team_roles_versions=team_roles_versions)


@roles_router.post("")
async def create_team_roles_endpoint(
    request: Request,
...
@roles_router.get("/{team_roles_id}")
async def get_team_roles_by_id_endpoint(
    request: Request,
    team_roles_id: str,
    db: DatabaseInfra = Depends(get_db_infra),
) -> ActiveTeamRolesResponse:
    ...
    if not result:
        raise HTTPException(
            status_code=404,
            detail="Team roles not found or do not belong to this team",
        )
...
@roles_router.post("/{team_roles_id}/activate")
async def activate_team_roles_endpoint(
    request: Request,
    team_roles_id: str,
    db: DatabaseInfra = Depends(get_db_infra),
    ...
    return ActivateTeamRolesResponse(
        activated=True,
        active_team_roles_id=team_roles_id,
    )
...
async def reset_team_roles_to_default_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to reload default team roles bundle: {exc}",
        ) from exc
...
async def deactivate_team_roles_endpoint(
    ...
    return DeactivateTeamRolesResponse(
        deactivated=True,
        active_team_roles_id=team_roles_version.id,
        version=team_roles_version.version,
    )
