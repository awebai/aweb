import { signedPayloadConversationStatus, verifyMessage, verifySignedPayload, } from "../identity/signing.js";
export async function fetchHistory(client, sessionId, unreadOnly = false, limit = 50, messageID) {
...
async function verifyChatMessage(msg) {
    if (msg.signed_payload && msg.signature && msg.from_did) {
        const status = await verifySignedPayload(msg.signed_payload, msg.signature, msg.from_did, msg.signing_key_id || "");
        if (status !== "verified")
            return status;
