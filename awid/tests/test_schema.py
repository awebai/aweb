...
@pytest.mark.asyncio
async def test_namespace_delegation_schema_is_append_only(awid_db_infra):
    db = awid_db_infra.get_manager("aweb")
    rows = await db.fetch_all(
        """
        SELECT table_name
    ...
    assert [row["table_name"] for row in rows] == sorted(
        [
            "namespace_delegation_heads",
            "namespace_delegation_entries",
            "namespace_delegation_signatures",
    ...
    columns = await db.fetch_all(
        ...
        WHERE table_schema = $1 AND table_name = 'dns_namespaces'
          AND column_name = ANY($2::text[])
        ORDER BY column_name
        """,
        db.schema or "public",
        ["active_delegation_hash", "last_verified_dns_ttl_seconds"],
    )
    assert [row["column_name"] for row in columns] == [
        "active_delegation_hash",
        "last_verified_dns_ttl_seconds",
    ]
    ...
    await db.execute(
        ...
        VALUES ('child.example.com', 'example.com', 1, $1, 'delegate', 'did:key:test')
        """,
        "sha256:" + "1" * 64,
    )
    await db.execute(
        ...
        VALUES ('child.example.com', 'example.com', 1, 'delegate',
                'did:key:test', NULL, $1, $2)
        """,
        b"{}",
        "sha256:" + "1" * 64,
    )
    with pytest.raises(QueryError):
        await db.execute(
            """
            UPDATE {{tables.namespace_delegation_entries}}
            SET canonical_payload = $1
            WHERE child_domain = 'child.example.com' AND sequence = 1
            """,
            b"changed",
        )
