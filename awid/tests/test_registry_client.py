...
@pytest.mark.asyncio
async def test_registry_client_carries_and_parses_namespace_delegation_assertion():
    ...
    payload = {
        ...
        "child_domain": "child.example.com",
        "child_controller_did": child_did,
        "sequence": 1,
        "previous_delegation_hash": None,
    }
    canonical = canonical_json_bytes(payload)
    assertion = {
        "payload": payload,
        "entry_hash": "sha256:" + hashlib.sha256(canonical).hexdigest(),
        "signatures": [{
            "controller_did": parent_did,
            "signature": sign_message(parent_key, canonical),
        }],
    }
    seen = {}
    ...
    def handler(request):
        ...
        return Response(
            ...
            json={
                ...
                "last_verified_at": "2026-08-26T18:00:00Z",
                "created_at": "2026-08-26T18:00:00Z",
                "delegation_chain": [assertion],
            },
        )
    ...
    try:
        namespace = await registry.register_namespace(
            "child.example.com",
            child_did,
            child_key,
            parent_signing_key=parent_key,
            delegation_assertion=assertion,
        )
    finally:
        await registry.aclose()

    assert seen["body"]["delegation_assertion"] == assertion
    assert namespace.delegation_chain[0].entry_hash == assertion["entry_hash"]


@pytest.mark.asyncio
@pytest.mark.parametrize(
    "failure", ["empty_chain", "noncontiguous", "head_mismatch", "active_revoke"]
)
async def test_python_log_collector_rejects_incomplete_authority(failure):
    signer_key, signer_public = generate_keypair()
    signer_did = did_from_public_key(signer_public)
    _, child_public = generate_keypair()
    child_did = did_from_public_key(child_public)
    payload = {
        "version": "awid.namespace-delegation.v1",
        "operation": "revoke" if failure == "active_revoke" else "delegate",
        "parent_domain": "example.com",
        "child_domain": "child.example.com",
        "child_controller_did": child_did,
        "sequence": 2 if failure == "noncontiguous" else 1,
        "previous_delegation_hash": (
            "sha256:" + "0" * 64 if failure == "noncontiguous" else None
        ),
    }
    canonical = canonical_json_bytes(payload)
    assertion = {
        ...
        "entry_hash": "sha256:" + hashlib.sha256(canonical).hexdigest(),
        "signatures": [{
            "controller_did": signer_did,
            "signature": sign_message(signer_key, canonical),
        }],
    }

    def handler(request):
        if request.url.path.endswith("/delegation-log"):
            head_hash = (
                "sha256:" + "f" * 64
                if failure == "head_mismatch"
                else assertion["entry_hash"]
            )
            return Response(200, json={
                "entries": [assertion], "has_more": False,
                "next_sequence": payload["sequence"], "next_cursor": None,
                "head_sequence": payload["sequence"], "head_hash": head_hash,
            })
        return Response(200, json={
            "namespace_id": "ns", "domain": "child.example.com",
            "controller_did": child_did, "verification_status": "verified",
            "created_at": "2026-08-26T18:00:00Z",
            "delegation_chain": [] if failure == "empty_chain" else [assertion],
        })
    ...
    try:
        with pytest.raises(RegistryError, match=(
            "omitted delegation chain" if failure == "empty_chain"
            else "not contiguous" if failure == "noncontiguous"
            else "active namespace" if failure == "active_revoke"
            else "final entry"
        )):
            await registry.collect_namespace_delegation_log(
                "child.example.com", max_restarts=0,
            )
    finally:
        await registry.aclose()
...
@pytest.mark.asyncio
async def test_python_log_collector_rejects_repeated_continuation_cursor():
    parent_key, parent_public = generate_keypair()
    parent_did = did_from_public_key(parent_public)
    _, first_public = generate_keypair()
    first_did = did_from_public_key(first_public)
    _, second_public = generate_keypair()
    second_did = did_from_public_key(second_public)
    ...
    delegate = signed({
        "version": "awid.namespace-delegation.v1", "operation": "delegate",
        "parent_domain": "example.com", "child_domain": "child.example.com",
        "child_controller_did": first_did, "sequence": 1,
        "previous_delegation_hash": None,
    })
    rotate = signed({
        "version": "awid.namespace-delegation.v1", "operation": "rotate",
        "parent_domain": "example.com", "child_domain": "child.example.com",
        "child_controller_did": second_did, "sequence": 2,
        "previous_delegation_hash": delegate["entry_hash"],
    })

    def handler(request):
        entry = rotate if request.url.params.get("cursor") else delegate
        return Response(200, json={
            "entries": [entry], "has_more": True,
            "next_sequence": entry["payload"]["sequence"],
            "next_cursor": "same-cursor", "head_sequence": 3,
            "head_hash": "sha256:" + "f" * 64,
        })

    registry = RegistryClient(registry_url="http://registry.test", transport=MockTransport(handler))
    try:
        with pytest.raises(RegistryError, match="continuation cursor repeated"):
            await registry.collect_namespace_delegation_log(
                "child.example.com", max_restarts=0,
            )
    finally:
...
@pytest.mark.asyncio
async def test_python_log_collector_accepts_valid_deleted_tombstone():
    parent_key, parent_public = generate_keypair()
    parent_did = did_from_public_key(parent_public)
    child_key, child_public = generate_keypair()
    child_did = did_from_public_key(child_public)
    delegate_payload = {
        "version": "awid.namespace-delegation.v1", "operation": "delegate",
        "parent_domain": "example.com", "child_domain": "child.example.com",
        "child_controller_did": child_did, "sequence": 1,
        "previous_delegation_hash": None,
    }
    delegate_bytes = canonical_json_bytes(delegate_payload)
    delegate_hash = "sha256:" + hashlib.sha256(delegate_bytes).hexdigest()
    delegate = {
        "payload": delegate_payload, "entry_hash": delegate_hash,
        "signatures": [{"controller_did": parent_did, "signature": sign_message(parent_key, delegate_bytes)}],
    }
    revoke_payload = {
        **delegate_payload, "operation": "revoke", "sequence": 2,
        "previous_delegation_hash": delegate_hash,
    }
    revoke_bytes = canonical_json_bytes(revoke_payload)
    revoke = {
        "payload": revoke_payload,
        "entry_hash": "sha256:" + hashlib.sha256(revoke_bytes).hexdigest(),
        "signatures": [{"controller_did": child_did, "signature": sign_message(child_key, revoke_bytes)}],
    }

    def handler(request):
        if request.url.path.endswith("/delegation-log"):
            return Response(200, json={
                "entries": [delegate, revoke], "has_more": False,
                "next_sequence": 2, "next_cursor": None,
                "head_sequence": 2, "head_hash": revoke["entry_hash"],
            })
        return Response(404, json={"detail": "Namespace not found"})

    registry = RegistryClient(registry_url="http://registry.test", transport=MockTransport(handler))
    try:
        page = await registry.collect_namespace_delegation_log("child.example.com")
    finally:
        await registry.aclose()
    assert page.entries[-1].payload["operation"] == "revoke"
    assert page.head_hash == revoke["entry_hash"]


@pytest.mark.asyncio
async def test_python_client_exact_dns_recovery_uses_strict_mode_and_dual_proof():
    ...
    def handler(request):
        if request.method == "POST":
            seen["body"] = json.loads(request.content)
            seen["proof"] = request.headers.get("X-AWEB-New-Controller-Authorization")
        return Response(200, json={
            "rollover_id": "rollover",
            "parent_domain": "exact.example",
            "old_controller_did": "did:key:z6Mkold",
            "new_controller_did": new_did,
            "recovery_mode": "exact_dns",
            "recovery_assertion": None,
            "state": "ready",
            "total_children": 0,
            "signed_children": 0,
    ...
    registry = RegistryClient(registry_url="http://registry.test", transport=MockTransport(handler))
    try:
        rollover = await registry.start_namespace_controller_rollover(
            "exact.example",
            current_controller_signing_key=new_key,
            new_controller_did=new_did,
            new_controller_signing_key=new_key,
            recovery_mode="exact_dns",
        )
        resumed = await registry.get_namespace_controller_rollover(
            "exact.example", "rollover"
        )
    finally:
        await registry.aclose()
    assert rollover.state == "ready"
    assert resumed == rollover
    assert seen["body"] == {
        "new_controller_did": new_did,
        "recovery_mode": "exact_dns",
    }
    assert seen["proof"].startswith("DIDKey ")
