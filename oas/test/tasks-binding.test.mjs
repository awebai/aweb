...
function temporaryDirectory() {
  const directory = realpathSync(mkdtempSync(join(tmpdir(), "aweb-oas-tasks-")));
...
test("real OAS spawn composes the aweb tasks integration into the instance", () => {
  ...
  const result = spawnSync(process.execPath, [oasCli(), "spawn", "developer", "--purpose", "tasks-binding", "--no-launch", "--json"], {
    cwd: repo,
