...
test("binding modes are an exact declared vocabulary with independent cleanup ownership", () => {
  ...
  assert.deepEqual(
    validateBindingSettings({ schema_version: 2, mode: "provision-disposable", minting_authority: "provisioner" }).mode,
    "provision-disposable",
  );
  assert.throws(() => validateBindingSettings({ schema_version: 2, mode: "provision-disposable", minting_authority: 123 }), /minting_authority/);
  assert.throws(() => validateBindingSettings({ schema_version: 2, mode: "provision-disposable", minting_authority: "provisioner", operation_id: "shared" }), /requires exactly.*minting_authority/);
  ...
  const firstIntent = pendingProvisionReceipt({ schema_version: 2, mode: "provision-disposable", minting_authority: "provisioner" });
  const secondIntent = pendingProvisionReceipt({ schema_version: 2, mode: "provision-disposable", minting_authority: "provisioner" });
