...
if [[ "$status" -ne 1 || "$seed_count" -ne 1 || "$diagnostic_count" -ne 1 ]] || \
   ! grep -Fq "FAILED: 1 failures," "$output"; then
  echo "FAIL: controller-key mutation did not make only the real absence assertion fail" >&2
  echo "journey status: $status; seed count: $seed_count; exact diagnostic count: $diagnostic_count" >&2
  tail -80 "$output" >&2
  exit 1
fi
