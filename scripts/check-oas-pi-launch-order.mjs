...
const oasRoot = process.env.OAS_TEST_ROOT ? realpathSync(resolve(process.env.OAS_TEST_ROOT)) : "";
const oasCli = oasRoot && join(oasRoot, "bin", "oas.mjs");
const contributedArgument = "--append-system-prompt";
const executionVariable = "OAS_RUN_REAL_PI_LAUNCH_ORDER";
const responseMarker = "OAS_PI_LAUNCH_ORDER_OK";
...
function createFixture(base, path, model = "") {
  ...
  write(join(capability, "oas.json"), JSON.stringify({
...
function parseSpawn(result) {
  ...
  try {
    ...
    throw new Error(`OAS spawn did not return JSON: ${error.message}\n${result.stdout}\n${result.stderr}`);
  }
  const spawned = document.schemaVersion === 1 ? document.result : document;
  if (!spawned?.home) throw new Error(`OAS spawn returned no instance home: ${result.stdout}`);
  return spawned;
}
...
function constructionCheck() {
  const base = realpathSync(mkdtempSync(join(tmpdir(), "aweb-oas-pi-order-construction-")));
  try {
    ...
    process.stdout.write("OAS Pi launch-order construction: PASS (pinned CLI --no-launch spawn)\n");
  } finally {
...
export function executionCheck({
  ...
  const base = realpathSync(mkdtempSync(join(tmpdir(), "aweb-oas-pi-order-execution-")));
  const home = join(base, "home");
  const piConfig = join(home, ".pi", "agent");
  const authPath = join(piConfig, "auth.json");
  const tmuxDirectory = join(base, "tmux");
  const tmuxSession = `aweb-oas-pi-order-${process.pid}`;
  const hostConfig = ambientEnv.PI_CODING_AGENT_DIR || join(ambientEnv.HOME || "", ".pi", "agent");
  const authSource = explicitAuthSource || ambientEnv.OAS_PI_LAUNCH_ORDER_AUTH || join(hostConfig, "auth.json");
  const model = explicitModel || ambientEnv.OAS_PI_LAUNCH_ORDER_MODEL || "openai-codex/gpt-5.4-mini:minimal";
  let executionEnv;
  let executionError;
  ...
  try {
    ...
    reportPass("OAS Pi launch-order execution: PASS (real Pi parsed and answered the spawned task)\n");
  } catch (error) {
...
export function main() {
  if (!oasRoot || !existsSync(oasCli)) {
    fail(`OAS_TEST_ROOT must contain the pinned OAS CLI: ${oasCli || "<unset>"}`);
  }

  try {
    constructionCheck();
    if (process.env[executionVariable] === "1") {
      process.stderr.write("OAS Pi launch-order execution: RUNNING (real runtime; network and model tokens will be used)\n");
      executionCheck();
    } else {
      process.stderr.write(
        `OAS Pi launch-order execution: SKIPPED (${executionVariable}=1 was not set); construction only, real Pi runtime was not executed.\n`,
      );
    }
  } catch (error) {
