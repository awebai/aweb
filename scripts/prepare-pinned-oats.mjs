...
if (!existsSync(requestedPinPath)) fail(`OATS pin does not exist: ${requestedPinPath}`);
...
try {
  ...
  fail(`cannot parse OATS pin ${pinPath}: ${error.message}`);
}

const repository = String(pin.repository || "").trim();
const commit = String(pin.commit || "").trim();
if (!repository) fail(`OATS pin ${pinPath} has no repository`);
if (!/^[0-9a-f]{40}$/.test(commit)) {
  fail(`OATS pin ${pinPath} must contain a full lowercase 40-hex commit`);
}

const markerName = "aweb-oats-pin-cache.json";
if (existsSync(target)) {
  if (lstatSync(target).isSymbolicLink()) fail(`refusing symlinked OATS cache ${target}`);
  ...
  try {
    ...
    fail(`refusing OATS cache ${target}; cache marker is unreadable`);
  }
  if (ownership.repository !== repository) {
    fail(`refusing OATS cache ${target}; cache marker names a different repository`);
  }
} else {
...
if (topLevel !== realpathSync(target)) {
  fail(`refusing OATS cache whose git top-level differs from its target: ${target}`);
}
...
if (origin !== repository) {
  fail(`refusing OATS cache ${target}; origin is ${origin}, expected ${repository}`);
}
...
if (head !== commit) fail(`prepared OATS checkout is ${head}, expected ${commit}`);
if (status) fail(`prepared OATS checkout is dirty: ${status}`);

process.stdout.write(`Prepared pinned OATS ${commit} at ${target}\n`);
