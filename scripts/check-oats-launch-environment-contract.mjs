...
const selectedRoot = process.env.OATS_TEST_ROOT;
const expectedContract = [
  ...
  ["bin/oats.mjs", "Requested launch environment:"],
];

function fail(root, missing) {
  const location = root || "<unset>";
  process.stderr.write(
    `ERROR: OATS_TEST_ROOT ${location} lacks the required launch-environment contract.\n`
    + "The aweb OATS seam tests depend on the launch-environment contract from upstream PR 48 (carried by the pinned OATS kernel); OAS releases before it do not contain this contract.\n"
    + "Point OATS_TEST_ROOT at the reviewed launch-environment-contract checkout before running test-oats.\n"
...
if (!selectedRoot) {
  fail(selectedRoot, ["OATS_TEST_ROOT"]);
} else {
  let root;
  try {
