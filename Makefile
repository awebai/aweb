...
help:
	...
	@echo "    release-plan                        compute and freeze the plan"
	@echo "    release-run PLAN_ID=.. PLAN_ARTIFACT_ID=.."
	@echo "                                        stage once, inspect exact bytes,"
	@echo "                                        publish those bytes, tag, verify, seal"
	@echo "    release-receipt                     read a sealed receipt"
	@echo "    See the 'release' skill for the hazards the driver cannot state:"
	...
	@echo ""
	@echo "  RETIRED - the -tag/-push pairs below no longer publish anything."
	@echo "  No publishing workflow triggers on a tag; they only move refs."
	@echo "  Use release-run instead. (aw-release.yml still tag-syncs cli/go"
	@echo "  to awebai/aw, which is legacy.) They remain only as the fallback"
	@echo "  while G5 measurements land; do not start a release with them:"
	@echo "    release-server-tag/-push, release-channel-tag/-push,"
	@echo "    release-awid-tag/-push, release-awid-pypi-tag/-push,"
	@echo "    release-a2a-gateway-tag/-push, release-cli-tag/-push"
	@echo ""
