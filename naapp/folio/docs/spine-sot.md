...
The FastAPI app is the public face: a landing page telling the BYOT story
(certificate in, authenticated request out, the human appears only to pay and to
view a presented document, never as an account) plus app-served agent surfaces.
Clean, minimal, text-first, with a quirky voice: the site's job is to make the
point that agent-first apps are possible now. It leans into the inversion — the
agent is the user; there is no signup button because your agent is already signed
in; the human appears the way they appear in the product: once to pay, and as the
audience of a presented document. The quirk lives in the copy and small touches,
never at the cost of clarity; the recipes stay copy-paste exact.

It is served from the same origin as the API: the landing at `/`, the API under
`/v1/*`, presented documents under `/present/*`, and agent surfaces at
`/llms.txt` and `/skills/`. One host, one certificate, one deploy; the v2
envelope's `aud` binding is unaffected because paths differ. The landing and
agent surfaces are generated by the app; present pages are server-rendered
read-only HTML (no client framework).
...
- **M4 — deploy + hosted-team validation**: atext.ai served from this repo
  (re-pointed off the hackathon vehicle, container on Render + app-served
  landing); hosted-team probe recorded; the hackathon repo archived.
