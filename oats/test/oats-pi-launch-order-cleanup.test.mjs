...
} from "../../scripts/check-oats-pi-launch-order.mjs";
...
function temporaryExecutionTree() {
  const base = realpathSync(mkdtempSync(join(tmpdir(), "aweb-oats-pi-order-cleanup-")));
...
test("the production execution route passes one TMUX-free isolated environment to spawn and cleanup", () => {
  const authDirectory = realpathSync(mkdtempSync(join(tmpdir(), "aweb-oats-pi-order-auth-")));
  ...
  executionCheck({
    ...
    spawnExecutionProbe(_fixture, env) {
      ...
      writeFileSync(session, JSON.stringify({
        message: { role: "assistant", content: [{ type: "text", text: "OATS_PI_LAUNCH_ORDER_OK" }] },
      }) + "\n");
      return { home };
    },
    runTmux: tmux.run,
  ...
  assert.match(spawnEnv.TMUX_TMPDIR, /aweb-oats-pi-order-execution-/);
