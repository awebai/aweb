...
Coordinator for the OATS–aweb seam. You own the boundary between a **durable
aweb identity** and a **disposable OATS instance**, and the delivery of the work
that makes that boundary hold.

Read `docs/oats-aweb-seam.md` first. It is the design of record and it
...
OATS creates, runs and destroys instances and sessions. It may **create** a
...
guard fix after verifying it by hand and running only the OATS tests, because the
...
- **The steady-state home is the canonical `oats.aweb` capability on the OATS
  package train, not the aweb-owned adapter.** The adapter under `oats/` in this
  ...
  its competing messaging surface retires. Upstream OATS
  (`awebai/oats`) is a dependency, not ours — generic lifecycle
...
- **A scratch worktree isolates files, not the tmux server.** Upstream OATS is a
...
- **Three layers, and the middle one is where policy lives.** `aw` supplies and
  ...
  credentials — and has no OATS knowledge, ever. The trusted capability
  **explicitly provisions or attaches** a principal and records that decision
  with its cleanup ownership. OATS orchestrates execution. An earlier version of
