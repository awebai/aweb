...
From `oats/agents/oats-coord/instances/alice` (never `cd` elsewhere before `aw`):

```
aw workspace status          # confirm Team=aweb-oats:aweb.ai Name=alice
aw task show aweb-oats-aaaa   # the epic
aw task list --status open | grep aaaa
```

Read `oats/docs/oats-aweb-seam.md` first — it is the design of record, and its
newest section ("Cleanup authority belongs to the spawner, never to the worker")
is today's main decision.

## Repository state

- **aweb**: work on `oats-coord/alice`, never on `main`. Keep local `main` as a
...
- **upstream OATS clone**: `~/prj/awebai/oas`. Branch `working` is HISTORY, not
...
- **The test pin resolves the same commit the binary runs**, through an
  aweb-controlled mirror `github.com/awebai/oats` rather than upstream. Upstream
...
- **Push to the upstream clone is deliberately blocked**:
  `remote.origin.pushurl = BLOCKED-open-a-PR-for-Pepe-instead-see-aweb-oats-aaaa`.
...
`aweb-oats:aweb.ai` is HOSTED, and the capability only mints under a local BYOT
...
`.56` (OATS_TEST_ROOT coupling fails legibly), `.28` (ordinary worker journey,
...
2. **`.47`** — ours to build, unblocked, blocks `.44`. AC already exposes the
   ...
   the SPAWNER's context, with OATS receiving no owner credential. Needs its own
...
We converge on Pepe's OATS package train and one canonical `oats.aweb`
capability. (2026-08-12 update: `aweb.identity-attach` was deleted outright and
replaced by the grants-based `aweb.identity` capability — see
`docs/oats-aweb-seam.md` and
...
comment on epic `aweb-oats-aaaa`.
...
3. **The train largely answers `.2`**: `oats.aweb` v1.8.0 declares
...
config selects canonical `oats.aweb`; the "deliberately distinct from
...
call, executed by the SPAWNER, with OATS receiving no owner credential. It blocks
`.44`.
...
`oats/docs/proposals/upstream-retire-blocks-on-incomplete-cleanup.md`,
