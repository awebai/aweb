...
def _wire_block(origin: str, tool: dict[str, Any]) -> str:
    if tool.get("auth") == "none":
        return f"curl -s {origin}{example_path(tool)}"
    locs = params_by_loc(tool)
...
def _operation(origin: str, tool: dict[str, Any], verb: str) -> str:
    req, opt = tool_params(tool)
    ...
    aw_examples = EXAMPLE_PATH_VALUES if is_public else None
    ...
    return f"""        <div class="cmd-panel op" id="op-{tool['name']}">
          ...
          <div class="cmd-list"><div class="cmd"><pre>{escape(_wire_block(origin, tool), quote=False)}</pre>{COPY_BTN}</div></div>
        </div>"""


def render_reference(manifest: dict[str, Any], site: SiteConfig, *, verb: str) -> str:
    ...
    public_ops = "\n".join(_operation(raw_origin, t, verb) for t in publics)
    team_ops = "\n".join(_operation(raw_origin, t, verb) for t in cert_tools(manifest))
