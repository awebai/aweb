...
func TestRetiredIdentityVocabularyIsConfinedToCompatibilityBoundaries(t *testing.T) {
	...
	allowed := map[string]map[string]int{
		...
		"cmd/aw/beads_mail_send.go": {
			...
			`flags.BoolVar(&beadsMailSendWisp, "wisp", true, "Mark ephemeral (beads default); meaningful once dual-write lands")`: 1,
			`flags.BoolVar(&beadsMailSendPermanent, "permanent", false, "Mark not ephemeral; meaningful once dual-write lands")`:  1,
		},
	}
