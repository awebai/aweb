...
func testCustodyService(t *testing.T, residentKey, sessionKey ed25519.PrivateKey) *custodyService {
	...
	return &custodyService{
		...
		grantStatus: func(ctx context.Context, grantID string) (custodyGrantStatus, error) {
			return custodyGrantStatus{Active: true, Scopes: []string{"mail.send", "chat.send"}, GrantDIDKey: sessionDID, TeamID: "backend:acme.com", Status: "active"}, nil
		},
	}
}
...
func TestCustodyGrantValidationFailsClosedOnEffectiveStatusExpiryBindingAndFreshness(t *testing.T) {
	...
	cases := []struct {
		...
		{"expired locally", custodyGrantStatus{Active: true, Status: "active", TeamID: req.TeamID, GrantDIDKey: req.SessionDIDKey, Scopes: []string{"mail.send"}, ExpiresAt: time.Now().Add(-time.Minute).UTC().Format(time.RFC3339)}, "grant_expired"},
		{"session mismatch", custodyGrantStatus{Active: true, Status: "active", TeamID: req.TeamID, GrantDIDKey: "did:key:zOther", Scopes: []string{"mail.send"}}, "grant_session_mismatch"},
		{"team mismatch", custodyGrantStatus{Active: true, Status: "active", TeamID: "ops:acme.com", GrantDIDKey: req.SessionDIDKey, Scopes: []string{"mail.send"}}, "grant_team_mismatch"},
	}
...
func TestCustodyStatusContainsOnlySafeReadiness(t *testing.T) {
	...
	if ready.Keys["signing_ready"] != true || ready.Freshness["last_checked_at"] == nil {
		t.Fatalf("status did not use real readiness check: %#v %#v", ready.Keys, ready.Freshness)
	}
...
func TestCustodyServeRefusesLiveSocketAndRemovesStaleSocket(t *testing.T) {
	_, residentKey, _ := ed25519.GenerateKey(nil)
	_, sessionKey, _ := ed25519.GenerateKey(nil)
	socket := filepath.Join(os.TempDir(), "aw-custody-test.sock")
	_ = os.Remove(socket)
