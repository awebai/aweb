...
func TestSetDIDDeliveryOriginAtSignsWithIdentityKey(t *testing.T) {
	t.Parallel()
	...
	did := ComputeDIDKey(pub)
	stableID := ComputeStableID(pub)
	deliveryOrigin := "https://identity.example"

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/did/"+stableID+"/delivery-origin" {
			t.Fatalf("path=%s", r.URL.Path)
		}
		...
		if !ed25519.Verify(pub, []byte(payload), sig) {
			t.Fatalf("invalid identity signature for payload %s", payload)
		}

		var body didDeliveryOriginUpdateRequest
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
			t.Fatalf("decode body: %v", err)
		}
		if body.DeliveryOrigin == nil || *body.DeliveryOrigin != deliveryOrigin {
			t.Fatalf("delivery_origin=%v want %s", body.DeliveryOrigin, deliveryOrigin)
		}
	...
	client := NewAWIDRegistryClient(server.Client(), nil)
	out, err := client.SetDIDDeliveryOriginAt(context.Background(), server.URL, stableID, deliveryOrigin, priv)
	if err != nil {
		t.Fatal(err)
	}
	if out.DeliveryOrigin != deliveryOrigin {
		t.Fatalf("DeliveryOrigin=%q want %q", out.DeliveryOrigin, deliveryOrigin)
	}
}
